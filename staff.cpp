...
public:
    ...
    Patient(string qn, int ewt, string st, string act)
...
private:
    ...
    void showQueue(const vector<Patient>& queue, const string& queueType) {
...
private:
    ...
    PerformanceData generateMockData(const string& date) {
        ...
        data.date = date;
        data.totalPatientsServed = 450; // As per the previous image
        data.averageWaitingTime = 22;   // As per the previous image
        return data;
    }
    ...
    void generatePerformanceReport() {
        ...
        while (!validDate) {
            ...
            // Check if the date is empty or in the wrong format
            if (date.empty() || date.length() != 10 || date[2] != '/' || date[5] != '/') {
                // Display error message without color
                cout << "Please Select a date before proceeding\n";
                continue; // Prompt for the date again
            }
        ...
        while (true) {
            ...
            // Options for Refresh, Back, and Export
            cout << "\nOptions:\n";
            ...
            if (choice == 1) {
                ...
                data = generateMockData(date); // Refresh with same date
            } else if (choice == 2) {
                cout << "Returning to staff main page...\n";
                break;
            } else if (choice == 3) {
                ...
                // In a real application, this would involve saving the report to a file
            }
        }
    }
